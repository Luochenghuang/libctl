...
(define (make-class type-name parent properties . opts)
  ; opts is a flat list of alternating (option-name option-value ...),
  ; e.g. ('after-copy 'mesh_after_copy 'after-destroy 'mesh_after_destroy).
...
(define (class-option class key)
  (let loop ((rest (class-options class)))
	 ...
	 (props (map
		 (lambda (p)
		   (cond
		    ...
		    (else p)))
		 non-option-forms))
	 (opt-args (apply append
			  (map (lambda (o)
				 `((quote ,(car o)) (quote ,(cadr o))))
			       option-forms))))
  `(begin
     ,@pdefs
     ,@ppdefs
